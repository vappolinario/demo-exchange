namespace Demo.Exchange.Domain.SeedWorks
{
    ...
    public class Result
    {
        protected Result() => Messages = new HashSet<string>();

        protected Result(string message) : this() => Messages.Add(message);

        protected Result(IEnumerable<string> messages) : this() => Messages.UnionWith(messages);
